...
- Money uses exact `int64` minor units, strict fixed-two-decimal external parsing, and checked arithmetic; financial values never pass through floating-point arithmetic.
- PostgreSQL is the authoritative financial coordination boundary. Same-wallet writes serialize through row locking, while different wallets remain independent.
- Wallet balances cannot become negative. Ledger arithmetic, uniqueness, and immutability are reinforced by database constraints and triggers.
- Wager idempotency is persistent across HTTP, SQS, process restarts, and multiple instances. The durable Inbox makes at-least-once inbound delivery safe.
- The transactional Outbox prevents publication before business-state commit and preserves stable event IDs across retries and republication.
- Missing references persist as `PENDING_REFERENCE`; any resolver instance can recover them with durable exponential backoff and TTL.
- For HTTP wagering requests, OIDC authentication makes the authenticated provider identity authoritative rather than trusting `providerId` from the request body.
- Reconciliation independently compares stored wallet balance with the append-only ledger from one consistent PostgreSQL snapshot.
...
| --- | --- |
| Malformed JSON, invalid money, or invalid wager contract | `400 Bad Request` |
| Missing or invalid token | `401 Unauthorized` |
| Provider/authorization mismatch | `403 Forbidden` |
| Wallet not found, where the operation addresses a wallet resource | `404 Not Found` |
| Idempotency or external transaction conflict | `409 Conflict` |
| First persisted `PROCESSED`, `REJECTED`, or `PENDING_REFERENCE` result | `201 Created` |
...
`PENDING_REFERENCE` is a successfully persisted wagering result, not a transport failure.
...
Four concrete event constructors own the wire event type and fix `version` to `1`: `WagerTransactionProcessed`, `WagerTransactionRejected`, `WagerTransactionPendingReference`, and `WalletBalanceChanged`. The envelope contains `eventId`, `eventType`, `aggregateId`, `correlationId`, optional `causationId`, `occurredAt`, `version`, and `data`. An `eventId` is generated once when the concrete event is constructed immediately before Outbox persistence, stored in both the Outbox row and envelope, and never regenerated by the publisher.
...
Executable integration coverage proves that:

- submitting the same BET 50 times concurrently produces one financial effect;
- two concurrent `80.00` BETs against `100.00` produce exactly one processed result, one rejection, and a final `20.00` balance;
- independent wallets progress concurrently, including across three independent application processes;
- the same logical operation across HTTP and SQS shares one persistent idempotency identity;
- a processed wager replays its original result through a completely new pgx pool and service;
- redelivery after database commit but failed `DeleteMessage` does not repeat financial processing;
- two Outbox publishers coordinate without concurrently claiming the same event;
- a successful publisher call followed by failed database confirmation leaves the event pending and republishes the same event ID;
- pending references recover across restart, and reconciliation checks stored balance against the ledger.
...
go test ./internal/application/wagering -run 'Test(ProcessedBetReplay|ProcessedWagerReplaySurvivesApplicationRestart|SameIdempotencyKey|SameBetFiftyTimes)' -count=1

# Typed event constructors and envelope contracts.
go test ./internal/application/eventpayload -count=1
...
go test ./internal/infrastructure/messaging/outbox -run 'Test(PublishBatch|ConcurrentPublishers|PendingEvent|PublishedEventIsRepublishedWithSameEventIDWhenConfirmationCommitFails)' -count=1

# SQS processing, visibility retry, and DLQ redrive.
go test ./internal/infrastructure/messaging/sqs -run 'TestConsumer' -count=1

# Explicit commit/delete, publish/confirmation, and restart replay windows.
go test ./internal/infrastructure/messaging/sqs -run '^TestConsumerDoesNotProcessBetAgainWhenDeleteFailsAfterCommit$' -count=1
go test ./internal/infrastructure/messaging/outbox -run '^TestPublishedEventIsRepublishedWithSameEventIDWhenConfirmationCommitFails$' -count=1
go test ./internal/application/wagering -run '^TestProcessedWagerReplaySurvivesApplicationRestart$' -count=1
...
GitHub Actions checks module tidiness and verification, formatting, and `go vet`; starts real PostgreSQL, Keycloak, and LocalStack dependencies; runs the full suite once, repeats it with `-count=3`, and runs the race suite. A separate job executes the dedicated three-process multi-instance harness.
...
- **Graceful shutdown:** workers stop accepting new work, in-flight context-aware processing drains within the configured lifecycle deadline, and PostgreSQL resources close after workers stop. If shutdown cancels processing, visibility is released only after the processor returns so another instance can safely redeliver the message.
