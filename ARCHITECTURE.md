...
The inbound FIFO producer contract sets `MessageGroupId = walletId` and `MessageDeduplicationId = messageId`. Grouping by `walletId` preserves command order for one wallet while allowing different wallets to proceed independently. The `messageId` remains the authoritative transport identity for Inbox registration and payload-hash comparison.
...
Each of the four concrete event constructors fixes its own `eventType` and version `1`; callers do not choose either value. The constructor generates the event ID once immediately before Outbox persistence, and that same ID is stored in both the Outbox row and envelope. Multiple publishers coordinate through PostgreSQL locking, and publisher retries always reuse the persisted identity. If sending succeeds but the process fails before marking the row published, the same event may be sent again. Downstream consumers must therefore deduplicate and tolerate at-least-once delivery.

Outbound FIFO publication sets `MessageGroupId = aggregateId` and `MessageDeduplicationId = eventId`. Grouping by `aggregateId` orders events for the same aggregate. Because `eventId` is persisted in the Outbox rather than generated by the publisher, recovery after a successful external send and failed PostgreSQL publication confirmation republishes the same payload with the same `eventId` and deduplication ID. This duplicate-send window is intentional at-least-once behavior.

Financial correctness does not depend on SQS FIFO deduplication. FIFO ordering and broker deduplication supplement PostgreSQL transactionality, Inbox `messageId` and payload-hash checks, wager idempotency, wallet locking/versioning, and persisted Outbox event identity; those mechanisms remain authoritative if a message is delivered again.
...
Production deployments should use the AWS SDK default credential chain with short-lived IAM role or workload-identity credentials, such as ECS task roles, EKS IRSA/Pod Identity, or EC2 instance roles. Static long-lived access keys are not recommended. LocalStack's `test/test` credentials are disposable local values.

When `SQS_ENDPOINT` is unset, the SDK uses the normal regional AWS endpoint. An explicit endpoint selects LocalStack or another intentional compatible service. Production deployments should use TLS, prevent public queue access, and enable server-side encryption as appropriate. Customer-managed KMS keys require corresponding KMS permissions, and a VPC endpoint is an optional network-hardening measure.
...
- **HTTP shutdown ordering:** Fx drains and stops workers before HTTP shutdown, so HTTP may remain available during the short worker-drain window.
...
- **Domain modeling:** `Wallet` validates creation/rehydration and controls debit/credit behavior, but its persisted fields are currently exported. `WagerTransaction` lifecycle is represented by persisted state plus application and database validation rather than a standalone rich aggregate with explicit transition methods. Ledger arithmetic and immutability are enforced primarily by application transaction flow and PostgreSQL constraints/triggers rather than a separate `WalletLedgerEntry` domain entity; database invariants remain authoritative. The low-level `NewMoney(int64, Currency)` constructor assumes trusted internal inputs and does not independently reject an empty currency; external parsing and supported application flows validate currency before financial processing.
